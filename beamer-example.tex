...
The implementation of the Algorithm is in Python. pyLouvain is package that is freely available although considering the task perfomed by the project it is tough to use pyLouvain directly. Hence modifications were made to pyLouvain and some part of the code was reused. 
\\
The input data structure was altered. The Input file is stored in a matrix and the transpose is used to get the node set. This is  used to for a edge dictionary. 
\\
...
\frame{
Alchemy needs three main units to form as an application namely: alchemy.css,
alchemy.js and data.
\\
Five simple steps to connect the JSON object to draw the graph.
\\
	\begin{figure}
	\centering
